...
public class SendIPThread implements Runnable {
	...
	public SendIPThread(int threadType, Socket accept, Map<String, String> paramsMap) {
		this.threadType = threadType;
		this.accept = accept;
		this.platformResourceCode = paramsMap.get("platformResourceCode");
		this.privateKey = paramsMap.get("privateKey");
		this.ipServerIP = paramsMap.get("ipServerIP");
		this.timeZone = paramsMap.get("timeZone");
		this.pid = paramsMap.get("pid");
		this.tsPort = paramsMap.get("tsPort");
		this.ipMikePlayPort = paramsMap.get("ipMikePlayPort");
		this.ipProtocal = paramsMap.get("ipProtocal");
	}
				...
				while (true) {
					if(accept != null && !accept.isClosed()){
						...
						if (("49").equals(answer.substring(0, 2))) {
							ipMikeAnswer(outputStream, ipServerIP, tsPort, pid); // TODO 6008端口  501pid 暂时写死
							//封装ipvo并发送开播指令
							getIpMikeInfo(answer,pid,platformResourceCode,timeZone,ipServerIP,ipMikePlayPort,ipProtocal);
						}
					} else {
	...
	private void getIpMikeInfo(String answer,String pid,String platformResourceCode,String timeZone,String ipPlayServerIP,String ipPlayPort,String ipProtocalSet){
		...
		String controlType = answer.substring(48,50); //开机：1；关机：2
		...
		ipVO.setIpPlayPort(ipPlayPort);
		ipVO.setServiceIp(ipPlayServerIP);
		...
		ipVO.setVolume((byte)(hexToTen(volume)));
		//TODO 时区
		...
		ipVO.setAudioPid(addZeroForNum(pid,4));
		String asciiUrl = UDPUtil.stringToAscii(audioUrl);
		int ascLen = asciiUrl.length();
		String len = getRealLenth(Integer.toHexString(13 + ascLen / 2)); //13为9位保留数据+4位pid
		ipVO.setLen(len);
		if("01".equals(controlType)){
			...
			//
			try {
				FfmpegUtil.ipMikeTS(ipVO);
				//FfmpegUtil.ipMikeTSTest(ipVO);
			} catch (Exception e) {
			...
			while (iterator.hasNext()) {
				IpVO stopIpVO = iterator.next();
				if (stopIpVO.getfId().equals(fId)) {
					iterator.remove();
				}
			}
		}
