...
public class FfmpegUtil {
	...
	public static void play(IpVO sendIpvo) throws IOException, InterruptedException {
		...
		if(Integer.parseInt(sendIpvo.getIpProtocalSet()) == 0){
			...
			command2 = "/usr/bin/cvlc " + broadcastTomcatPath + "/webapps/ROOT/upload/audio/" + audioName
					+ " -vv -I dummy --sout '#standard{mux=ts{pid-audio=" + audioPid + "},access=http,dst=:"
					+ ipPlayPort + "/vlc}' --play-and-exit";
			
		/*	command2 = "/usr/bin/cvlc " + broadcastTomcatPath + "/webapps/ROOT/upload/audio/" + audioName
					+ " -vv -I dummy --sout '#standard{mux=ts,access=http,dst=" + clientIp + ":"
					+ ipPlayPort + "/vlc}' --play-and-exit";*/
		}else{
			command2 = "/usr/bin/cvlc "+ broadcastTomcatPath + "/webapps/ROOT/upload/audio/"+audioName + " -vv -I dummy --sout=#rtp{sdp=rtsp://:"+ipPlayPort+"/vlc} --play-and-exit";
		}
	...
	public static void ipMikeTS(IpVO sendIpvo) throws IOException, InterruptedException {
		Integer audioPid = hexToTen(sendIpvo.getAudioPid());
		String command2="/usr/bin/cvlc rtp://@:"+ sendIpvo.getIpMikeTSport() +" --sout '#standard{mux=ts{pid-audio=" + audioPid + "},access=http,dst=:"
					+ sendIpvo.getIpPlayPort() + "/vlc}' --play-and-exit";
		String[] cmd = new String[] { "sh", "-c", command2 };
		logger.info(command2);
		Process process = Runtime.getRuntime().exec(cmd);
		dealStream(process);
		process.waitFor(); // 加上这句，系统会等待转换完成。不加，就会在服务器后台自行转换。
		logger.info("ip话筒TS转发执行成功");
	}
	public static void ipMikeTSTest(IpVO sendIpvo) throws IOException, InterruptedException {
		Integer audioPid = hexToTen(sendIpvo.getAudioPid());
		String command2="D:/vlc/vlc.exe rtp://@:5000 :sout=#http{mux=ts{pid-audio="+audioPid+"},dst=:"+ sendIpvo.getIpPlayPort() +"/vlc} :no-sout-all :sout-keep";
		logger.info(command2);
