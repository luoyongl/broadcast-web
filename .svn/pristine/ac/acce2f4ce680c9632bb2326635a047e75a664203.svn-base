package cn.wtu.broadcast.parent.utils.string;
...
public class StringUtil extends StringUtilParent {
	...
	/**
	 * @Title: splitPics @Description: 将图片返回成list @param @param
	 *         pics @param @return 参数 @return List<String> 返回类型 @throws
	 */
	public static List<String> splitPics(String pics) {
		List<String> picList = new ArrayList<String>();
		if (StringUtils.isNotBlank(pics)) {
			String[] picArr = pics.split("\\|\\|");// 双竖线分拆
			for (int i = 0; i < picArr.length; i++) {
				if (StringUtil.isNotBlank(picArr[i])) {
	...
	public static String getOrderNo() {
		...
		String no = sdf.format(new Date()) + String.valueOf((int) ((Math.random() * 9 + 1) * 1000));
		return no;
	}
	...
	public static String getQRCode() {
		...
		String no = sdf.format(new Date()) + String.valueOf((int) ((Math.random() * 9 + 1) * 10000000));
		return no;
	}

	/**
	 * 将vo对象对应的属性名转换为对应数据库中的字段名
	 * @author AYY
	 * @param fieldName
	 * @return
	 *
	...
	public static String FieldNameToColumnName(String fieldName){
		...
		for(int i=0;i<fieldName.length();i++){
			if(fieldName.charAt(i)>='A' && fieldName.charAt(i)<='Z'){
				sb.append('_');
				sb.append((char)((int)fieldName.charAt(i)+32));
				continue;
			}
